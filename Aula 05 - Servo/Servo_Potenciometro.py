...
class Servo:
    ...
    def __initialise(self, pin):
        ...
        self.__angle_conversion_factor = (
            self.__max_u10_duty - self.__min_u10_duty) / (self.max_angle - self.min_angle)
...
def converter(x):
    return Lerp(0, 180, x/4095)


motor = Servo(pin=23)  # frequência de servomecanismo
...
while True:
    angle = converter(pot.read())
    print(angle, pot.read())
    motor.move(angle)
    sleep(0.1)
